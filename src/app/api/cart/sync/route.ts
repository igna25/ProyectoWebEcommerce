...
export async function POST(request: Request) {
  try {
    const { userId: rawUserId, items: rawItems } = await request.json() as {
      userId: string,
      items: Array<{ productid: string, quantity: number, productprice?: number }>
    }
    ...
    const itemsWithServerPrices = await Promise.all(requestedItems.map(async (requestedItem) => {
      const product = await productsRepository.getProductById(requestedItem.productid)
      return {
    ...
    const newCartTotal = itemsWithServerPrices.reduce((sum, item) => sum + item.price * item.quantity, 0)

    if (!userCart) {
      const createdCartId = await cartsRepository.createCart(userId, newCartTotal, 'sync')
    ...
    if (userCart) {
      for (const normalizedItem of itemsWithServerPrices) {
        await orderItemsRepository.createOrderItem(userCart.id, normalizedItem.productid, normalizedItem.quantity, normalizedItem.price)
      }
    ...
    return NextResponse.json({ msg: 'Error trying to sync cart' }, { status: 500 })
  }
}
